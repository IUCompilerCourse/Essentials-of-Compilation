...
\HalfTitle{Essentials of Compilation, Python Edition}
...
\Title{Essentials of Compilation, Python Edition}
...
%
The main difference compared to the concrete syntax of \LangXInt{}
(Figure~\ref{fig:x86-int-concrete}) is that we do not yet include a
way to label instructions but instead recommend inserting the
\key{main}, \key{start}, and \key{conclusion} labels when printing the
final x86 program.
%
\fi}
...
register allocation (Chapter~\ref{ch:register-allocation-Rvar}).
...
of each of the compiler passes in Figure~\ref{fig:Rvar-passes}.
...
\path[->,bend left=15] (Rvar) edge [above] node {\ttfamily\footnotesize uniquify} (Rvar-2);
\path[->,bend left=15] (Rvar-2) edge [above] node {\ttfamily\footnotesize remove\_complex.} (Rvar-3);
\path[->,bend left=15] (Rvar-3) edge [right] node {\ttfamily\footnotesize explicate\_control} (Cvar-1);
...
\path[->,bend left=15] (Rvar-1) edge [above] node {\ttfamily\footnotesize remove\_complex.} (Rvar-2);
\path[->,bend right=15] (Rvar-2) edge [left] node {\ttfamily\footnotesize select\_instr.} (x86-1);
...
variables, wheras \code{y} and \code{tmp\_1} are move related to
...
\path[->,bend left=15] (Rif-4) edge [right] node {\ttfamily\footnotesize explicate\_control} (C-1);
...
continuation is passed it into two or more recursive calls). However,
...
a list of statement. We use the term \emph{promise} to refer to either
a list of statements or a function.  To uniformly deal with promises,
we define the following \code{force} function that checks whether its
input is a function and then either 1) calls the function, or 2)
returns the input.
\begin{lstlisting}
def force(promise):
...
that creates a basic block for the continuations (if there is not
