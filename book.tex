...
recurisvely calls \code{interp\_Lvar\_exp} again on the argument of
...
\racket{The largest changes occur in \code{explicate\_control}, to
  ...
  (Section~\ref{sec:explicate-control-Lif}).}
%
...
and
\racket{\code{type-check-Lif.rkt}}\python{\code{type\_check\_Pif.py}}
of the support code.
%
...
{\if\edition\pythonEd
\begin{lstlisting}
class TypeCheckPif(TypeCheckLvar):
  def type_check_exp(self, e, env):
    match e:
...
\path[->,bend left=15] (Lif-1) edge [above] node {\ttfamily\footnotesize type\_check} (Lif-2);
\path[->,bend left=15] (Lif-2) edge [above] node {\ttfamily\footnotesize shrink} (Lif-3);
\path[->,bend left=15] (Lif-3) edge [above] node {\ttfamily\footnotesize remove\_complex.} (Lif-4);
\path[->,bend left=15] (Lif-4) edge [right] node {\ttfamily\footnotesize explicate\_control} (C-1);
